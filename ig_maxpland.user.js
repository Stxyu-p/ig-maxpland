    ...
    async function runRelationshipScan() {
        ...
        try {
            ...
            const lostFollowers = (prev?.follower_ids || []).filter(id => !followerIdSet.has(String(id))).map(id => ({
                pk: String(id), id: String(id), username: prev?.follower_usernames?.[String(id)] || prev?.usernames?.[String(id)] || `user_${id}`, full_name: 'เลิกติดตามหลังจากสแกนครั้งก่อน', profile_pic_url: ''
            }));
